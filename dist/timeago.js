...
;(function(date_time){
    ...
    for(var i = 0; i < date_time.length; i++) {
        (function(timeData){
            ...
            var hours = (seconds / 3600);
            if(seconds < 60 && minutes < 1) {
                date_time[i].innerText = (seconds === 1 ? "in " + Math.round(seconds) + " second" : "in " + Math.round(seconds) + " seconds");
                return;
            }
            if(minutes < 60 && hours < 1) {
                date_time[i].innerText = (minutes === 1 ? "in " + Math.round(minutes) + " minute" : "in " + Math.round(minutes) + " minutes");
            	return;
            }
            if(hours > 24){
                var days = hours / 24;
                if (days > 30) {
                    ...
                    if (month > 12) {
                        var years = month / 12;
                        if (years > 0) {
                            date_time[i].innerText = (years === 1 ? "in " + Math.ceil(years) + " year" : "in " + Math.ceil(years) + " years"); 
                            return;
                        }
                    }
                    date_time[i].innerText = ("in " + Math.round(month) + " month"); 
                    return;
                }
                date_time[i].innerText = (days === 1 ? "in " + Math.round(days) + " day" : "in " + Math.round(days) + " days");
                return;
            } else {
                date_time[i].innerText = (hours === 1 ? "in " + Math.round(hours) + " hour" : "in " + Math.round(hours) + " hours");
                return;
            }
        })(date_time[i].innerText);    
    }
})(document.getElementsByClassName('timeago-future'));